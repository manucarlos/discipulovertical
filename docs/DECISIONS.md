...
- **Contraste WCAG AA calculado a partir da paleta** (`src/lib/contrast.test.ts`, ~35 pares): tema claro, modo escuro da leitura, selos coloridos e barras. Achou o botão de destaque do modo escuro (branco sobre a cor clara); agora existe a cor `onBrand`.
- **Marca personalizável pelo painel** (Administração > Marca, RF-33): as 3 cores escolhidas viram a paleta completa por `derivePalette` (com o contraste WCAG AA garantido; cor clara demais é escurecida e avisada), gravada em `church_brand` (tabela comum, sem Storage). O logotipo enviado gera todas as imagens (`processLogo`, com `sharp`) e fica em `church_assets` em base64, servido por `/marca/*`. O `brand.ts` continua como **padrão de segurança**: sem personalização, ou com o banco fora do ar, vale ele. Só entra no CSS uma paleta validada (`#rrggbb` nos 9 papéis, no servidor e por restrição no banco).
- **Paleta padrão em um lugar só** (`src/lib/brand.ts`, guia em [MARCA.md](MARCA.md)). As cores têm **papel** (`brand`, `tint`, `muted`...) e não nome de cor, para a troca ser segura. O layout as grava como variáveis CSS na página; o `globals.css` só dá nomes ao Tailwind, e a barra do navegador, o manifesto e o PDF do certificado leem o mesmo arquivo. Um teste falha se outro arquivo escrever uma cor da paleta à mão, e o teste de contraste avisa qual par de cores ficou ilegível. Por que TypeScript e não CSS: dá para reutilizar a mesma cor fora do CSS (PDF, manifesto) sem ler arquivo em tempo de execução. Paleta atual, do logotipo: laranja `#c14602` e preto `#171717`.
