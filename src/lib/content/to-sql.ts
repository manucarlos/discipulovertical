...
/**
 * Gera um SQL que cria ciclos e lições como RASCUNHO. É seguro repetir:
 *   - ciclos e lições que já existem são ignorados (nunca sobrescreve o que o pastor editou);
...
export function buildImportSql(cycles: ParsedCycle[], options: ImportSqlOptions = {}): string {
  ...
  out.push(
    ...
    `-- ${cycles.length} ciclo(s), ${lessonCount} lição(ões). Lições novas entram como ${options.publish ? "PUBLICADAS (exceto as com [PREENCHER])" : "RASCUNHO"}.`,
  ...
  for (const cycle of cycles) {
    out.push(
      `insert into public.cycles (slug, title, description, position, planned_weeks)`,
      `values (${sqlText(cycle.slug)}, ${sqlText(cycle.title)}, ${sqlText(cycle.description)}, ${cycle.number}, ${cycle.plannedWeeks})`,
      `on conflict (slug) do nothing;`,
      "",
    );
    for (const lesson of cycle.lessons) out.push(lessonBlock(cycle, lesson, options), "");
  }
...
function lessonBlock(cycle: ParsedCycle, lesson: ParsedLesson, options: ImportSqlOptions): string {
  ...
  const quiz = lesson.quiz.map(
    (q) =>
      ...
      `    values (v_lesson, ${q.position}, ${sqlText(q.prompt)}, ${sqlText(JSON.stringify(q.options))}::jsonb, ${sqlText(q.correct)}, ${sqlText(q.explanation)});`,
  );
  ...
  return [
    ...
    `begin`,
    `  select id into v_cycle from public.cycles where slug = ${sqlText(cycle.slug)};`,
    `  if exists (select 1 from public.lessons where slug = ${sqlText(lesson.id)}) then`,
    `    raise notice '${lesson.id}: já existe, ignorada';`,
    `  else`,
    `    insert into public.lessons (cycle_id, slug, title, objective, position, estimated_minutes, tags,`,
    `                                key_verse_ref, has_placeholders, status, current_version_id)`,
    `    values (v_cycle, ${sqlText(lesson.id)}, ${sqlText(lesson.title)}, ${sqlText(lesson.objective)}, ${lesson.order},`,
    ...
    `    values (v_version, v_lesson, ${sqlText(JSON.stringify(lesson.content))}::jsonb, 'Importação inicial do handoff');`,
    ``,
    ...(hasInternal
      ? [
          `    insert into public.lesson_internal_notes (lesson_id, pastoral_review_note, video_suggestion, draft_notice, button_suggestion)`,
          `    values (v_lesson, ${sqlNullableText(internal.pastoralReviewNote)}, ${sqlNullableText(internal.videoSuggestion)}, ${sqlNullableText(internal.draftNotice)}, ${sqlNullableText(internal.buttonSuggestion)});`,
          ``,
        ]
      : []),
    ...quiz,
