...
export interface LibrarySqlOptions {
  ...
  churchName?: string;
  /** O slug da igreja dona da biblioteca (banco único multi-igreja, migração 0026). Obrigatório. */
  churchSlug: string;
}
...
export function buildLibrarySql(lessonTemplates: LibraryLesson[], tracks: LibraryTrack[], options: LibrarySqlOptions): string {
  ...
  const out: string[] = [
    ...
    `-- ${lessons.length} lição(ões) e ${tracks.length} trilha(s), para a igreja "${options.churchSlug}".`,
    `-- Tudo entra como ${options.publish ? "PUBLICADO (exceto o que tem [PREENCHER])" : "RASCUNHO"}.`,
  ...
  for (const lesson of lessons) {
    ...
    out.push(
      ...
      "begin",
      `  if exists (select 1 from public.lessons where church_id = v_church and slug = ${sqlText(lesson.slug)}) then`,
      `    raise notice '${lesson.slug}: já existe para esta igreja, ignorada';`,
      "  else",
      "    insert into public.lessons (church_id, kind, cycle_id, slug, title, objective, position, estimated_minutes, tags,",
      "                                key_verse_ref, sensitive, themes, has_placeholders, status, current_version_id)",
      `    values (v_church, 'library', null, ${sqlText(lesson.slug)}, ${sqlText(lesson.title)}, ${sqlText(lesson.objective)}, ${lesson.n},`,
      ...
      `    values (v_church, v_version, v_lesson, ${sqlText(json)}::jsonb, 'Importação da biblioteca');`,
      "",
      "    insert into public.lesson_internal_notes (church_id, lesson_id, pastoral_review_note, draft_notice)",
      `    values (v_church, v_lesson, ${sqlText(lesson.reviewNote)}, ${sqlText(DRAFT_NOTICE)});`,
  ...
  for (const track of tracks) {
    ...
    out.push(
      ...
      "  v_days int;",
      "begin",
      `  if exists (select 1 from public.tracks where church_id = v_church and title = ${sqlText(track.title)}) then`,
      `    raise notice '${track.title.replace(/'/g, "''")}: trilha já existe, ignorada';`,
      "  else",
      "    insert into public.tracks (church_id, title, description, themes, status)",
      `    values (v_church, ${sqlText(track.title)}, ${sqlText(track.description)}, ${sqlTextArray(track.themes)}, 'draft')`,
      "    returning id into v_track;",
      "    insert into public.track_days (church_id, track_id, day_number, lesson_id)",
      "    select v_church, v_track, d.n, l.id",
      `    from (values ${days.join(", ")}) as d(n, slug)`,
      "    join public.lessons l on l.slug = d.slug and l.church_id = v_church;",
