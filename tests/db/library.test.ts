...
describe("importação da biblioteca (rascunho)", () => {
  it("cria as 32 lições da biblioteca e as 7 trilhas, tudo como rascunho, sem ciclo", async () => {
    ...
    expect(lessons.filter((l) => l.sensitive).map((l) => l.position)).toEqual([9, 10, 12, 13, 14, 16, 17, 28, 29, 30, 32]);
    expect(lessons.filter((l) => l.has_placeholders).map((l) => l.position)).toEqual([7, 9, 10, 12, 13, 14, 16, 17, 28, 32]);
    ...
    expect(tracks).toEqual([
      ...
      { title: "Vida em Família", status: "draft", dias: 4 },
    ]);
  });
  ...
  it("é seguro repetir: nada é duplicado nem sobrescrito", async () => {
    ...
    await db.exec(buildLibrarySql(LIBRARY_LESSONS, LIBRARY_TRACKS, { churchSlug: "vertical-church" }));
    expect((await q("select 1 from public.lessons")).length).toBe(32);
    expect((await q("select 1 from public.tracks")).length).toBe(7);
...
describe("importação com --publish (homologação)", () => {
  it("publica só o que não tem [PREENCHER]; trilhas com lição bloqueada continuam rascunho", async () => {
    ...
    try {
      ...
      expect(published.rows).toHaveLength(32 - 10);
      const tracks = await other.query<{ title: string; status: string }>("select title, status from public.tracks order by created_at");
      // Todas as trilhas incluem pelo menos uma lição com [PREENCHER] (7, 9, 10, 12, 13, 14, 16, 17, 28 ou 32), exceto Maturidade.
